...
var $ = (O, X) => Z(O, "name", { value: X, configurable: !0 });
import { ExternalTokenizer as S, styleTags as _, tags as Q, LRParser as q, LRLanguage as l, indentNodeProp as w, continuedIndent as s, foldNodeProp as V, foldInside as R, LanguageSupport as g } from "../../../editor.js";
const b = 1, y = 2, T = 3, c = 4, v = 5, t = 98, p = 101, d = 102, W = 114, f = 69, i = 48, Y = 46, x = 43, U = 45, z = 35, o = 34, h = 124, G = 60, m = 62;
function a(O) {
  return O >= 48 && O <= 57;
}
$(a, "isNum");
...
const u = new S((O, X) => {
  if (a(O.next)) {
    ...
    while (n(O.next));
    if (O.next == Y) {
      if (P = !0, O.advance(), a(O.next))
        do
          O.advance();
        while (n(O.next));
      else if (O.next == Y || O.next > 127 || /\w/.test(String.fromCharCode(O.next)))
        return;
    }
    if (O.next == p || O.next == f) {
      if (P = !0, O.advance(), (O.next == x || O.next == U) && O.advance(), !n(O.next)) return;
      do
        O.advance();
      while (n(O.next));
    }
    if (O.next == d) {
      let e = O.peek(1);
      if (e == i + 3 && O.peek(2) == i + 2 || e == i + 6 && O.peek(2) == i + 4)
        O.advance(3), P = !0;
      else
        return;
    }
    P && O.acceptToken(v);
  } else if (O.next == t || O.next == W) {
    if (O.next == t && O.advance(), O.next != W) return;
    O.advance();
    let P = 0;
    for (; O.next == z; )
      P++, O.advance();
  ...
  O.next == h && O.acceptToken(b, 1);
}), j = new S((O) => {
  O.next == G ? O.acceptToken(y, 1) : O.next == m && O.acceptToken(T, 1);
}), E = _({
  "const macro_rules struct union enum type fn impl trait let static": Q.definitionKeyword,
  "mod use crate": Q.moduleKeyword,
  "pub unsafe async mut extern default move": Q.modifier,
  "for if else loop while match continue break return await": Q.controlKeyword,
  "as in ref": Q.operatorKeyword,
  "where _ crate super dyn": Q.keyword,
  self: Q.self,
  String: Q.string,
  Char: Q.character,
  RawString: Q.special(Q.string),
  Boolean: Q.bool,
  Identifier: Q.variableName,
  "CallExpression/Identifier": Q.function(Q.variableName),
  BoundIdentifier: Q.definition(Q.variableName),
  "FunctionItem/BoundIdentifier": Q.function(Q.definition(Q.variableName)),
  LoopLabel: Q.labelName,
  FieldIdentifier: Q.propertyName,
  "CallExpression/FieldExpression/FieldIdentifier": Q.function(Q.propertyName),
  Lifetime: Q.special(Q.variableName),
  ScopeIdentifier: Q.namespace,
  TypeIdentifier: Q.typeName,
  "MacroInvocation/Identifier MacroInvocation/ScopedIdentifier/Identifier": Q.macroName,
  "MacroInvocation/TypeIdentifier MacroInvocation/ScopedIdentifier/TypeIdentifier": Q.macroName,
  '"!"': Q.macroName,
  ...
  "=": Q.definitionOperator,
  ".. ... => ->": Q.punctuation,
  "( )": Q.paren,
  "[ ]": Q.squareBracket,
  "{ }": Q.brace,
  ". DerefOp": Q.derefOperator,
  "&": Q.operator,
  ", ; ::": Q.separator,
  "Attribute/...": Q.meta
}), I = { __proto__: null, self: 28, super: 32, crate: 34, impl: 46, true: 72, false: 72, pub: 88, in: 92, const: 96, unsafe: 104, async: 108, move: 110, if: 114, let: 118, ref: 142, mut: 144, _: 198, else: 200, match: 204, as: 248, return: 252, await: 262, break: 270, continue: 276, while: 312, loop: 316, for: 320, macro_rules: 327, mod: 334, extern: 342, struct: 346, where: 364, union: 379, enum: 382, type: 390, default: 395, fn: 396, trait: 412, use: 420, static: 438, dyn: 476 }, A = q.deserialize({
  version: 14,
  ...
  tokenizers: [k, j, u, 0, 1, 2, 3],
  topRules: { SourceFile: [0, 8] },
  specialized: [{ term: 281, get: /* @__PURE__ */ $((O) => I[O] || -1, "get") }],
  tokenPrec: 15596
}), J = /* @__PURE__ */ l.define({
  name: "rust",
  parser: /* @__PURE__ */ A.configure({
    props: [
      /* @__PURE__ */ w.add({
        IfExpression: /* @__PURE__ */ s({ except: /^\s*({|else\b)/ }),
        "String BlockComment": /* @__PURE__ */ $(() => null, "String BlockComment"),
        AttributeItem: /* @__PURE__ */ $((O) => O.continue(), "AttributeItem"),
        "Statement MatchArm": /* @__PURE__ */ s()
      }),
      /* @__PURE__ */ V.add((O) => {
        if (/(Block|edTokens|List)$/.test(O.name))
          return R;
        if (O.name == "BlockComment")
          return (X) => ({ from: X.from + 2, to: X.to - 2 });
      })
    ]
  }),
  languageData: {
