...
var a = (O, Q) => t(O, "name", { value: Q, configurable: !0 });
import { ExternalTokenizer as P, styleTags as l, tags as $, LRParser as x, LRLanguage as R, indentNodeProp as V, continuedIndent as e, delimitedIndent as r, foldNodeProp as U, foldInside as v, html as u, LanguageSupport as b, parseMixed as m } from "../../../editor.js";
const p = 1, k = 2, c = 263, Y = 3, Z = 264, X = 265, w = 266, _ = 4, h = 5, j = 6, G = 7, d = 8, f = 9, g = 10, E = 11, I = 12, N = 13, F = 14, C = 15, L = 16, A = 17, H = 18, K = 19, D = 20, B = 21, M = 22, J = 23, OO = 24, $O = 25, QO = 26, iO = 27, yO = 28, aO = 29, zO = 30, SO = 31, PO = 32, WO = 33, eO = 34, TO = 35, sO = 36, XO = 37, dO = 38, nO = 39, qO = 40, oO = 41, tO = 42, lO = 43, xO = 44, RO = 45, VO = 46, rO = 47, UO = 48, vO = 49, uO = 50, bO = 51, mO = 52, pO = 53, kO = 54, cO = 55, YO = 56, ZO = 57, wO = 58, _O = 59, hO = 60, jO = 61, T = 62, GO = 63, fO = 64, gO = 65, EO = {
  abstract: _,
  and: h,
  array: j,
  as: G,
...
a(o, "isASCIILetter");
function z(O) {
  return O == 95 || O >= 128 || o(O);
}
a(z, "isIdentifierStart");
function s(O) {
...
const IO = {
  ...
  if (O.next == 40) {
    ...
    let i = "", y;
    for (; o(y = O.peek(Q)); )
      i += String.fromCharCode(y), Q++;
    for (; q(O.peek(Q)); ) Q++;
    O.peek(Q) == 41 && IO[i.toLowerCase()] && O.acceptToken(p);
  } else if (O.next == 60 && O.peek(1) == 60 && O.peek(2) == 60) {
    for (let y = 0; y < 3; y++) O.advance();
    for (; O.next == 32 || O.next == 9; ) O.advance();
    let Q = O.next == 39;
    if (Q && O.advance(), !z(O.next)) return;
    let i = String.fromCharCode(O.next);
    for (; O.advance(), !(!z(O.next) && !(O.next >= 48 && O.next <= 55)); )
      i += String.fromCharCode(O.next);
    if (Q) {
      if (O.next != 39) return;
      O.advance();
    }
    if (O.next != 10 && O.next != 13) return;
    for (; ; ) {
      let y = O.next == 10 || O.next == 13;
      if (O.advance(), O.next < 0) return;
      if (y) {
        for (; O.next == 32 || O.next == 9; ) O.advance();
        let S = !0;
        for (let W = 0; W < i.length; W++) {
          if (O.next != i.charCodeAt(W)) {
            S = !1;
            break;
          }
          O.advance();
        }
        if (S) return O.acceptToken(k);
      }
    }
  }
}), FO = new P((O) => {
  O.next < 0 && O.acceptToken(w);
}), CO = new P((O, Q) => {
  O.next == 63 && Q.canShift(X) && O.peek(1) == 62 && O.acceptToken(X);
});
function LO(O) {
  let Q = O.peek(1);
  if (Q == 110 || Q == 114 || Q == 116 || Q == 118 || Q == 101 || Q == 102 || Q == 92 || Q == 36 || Q == 34 || Q == 123)
    return 2;
  if (Q >= 48 && Q <= 55) {
    let i = 2, y;
    for (; i < 5 && (y = O.peek(i)) >= 48 && y <= 55; ) i++;
    return i;
  }
  if (Q == 120 && s(O.peek(2)))
    return s(O.peek(3)) ? 4 : 3;
  if (Q == 117 && O.peek(2) == 123)
    for (let i = 3; ; i++) {
      let y = O.peek(i);
      if (y == 125) return i == 2 ? 0 : i + 1;
      if (!s(y)) break;
    }
  return 0;
}
...
const AO = new P((O, Q) => {
  let i = !1;
  for (; !(O.next == 34 || O.next < 0 || O.next == 36 && (z(O.peek(1)) || O.peek(1) == 123) || O.next == 123 && O.peek(1) == 36); i = !0) {
    if (O.next == 92) {
      let y = LO(O);
      if (y) {
        if (i) break;
        return O.acceptToken(Y, y);
      }
    } else if (!i && (O.next == 91 || O.next == 45 && O.peek(1) == 62 && z(O.peek(2)) || O.next == 63 && O.peek(1) == 45 && O.peek(2) == 62 && z(O.peek(3))) && Q.canShift(Z))
      break;
    O.advance();
  }
  i && O.acceptToken(c);
...
}), KO = { __proto__: null, static: 311, STATIC: 311, class: 333, CLASS: 333 }, DO = x.deserialize({
  version: 14,
  ...
  tokenizers: [NO, AO, CO, 0, 1, 2, 3, FO],
  topRules: { Template: [0, 72], Program: [1, 232] },
  dynamicPrecedences: { 284: 1 },
  specialized: [{ term: 81, get: /* @__PURE__ */ a((O, Q) => n(O) << 1, "get"), external: n }, { term: 81, get: /* @__PURE__ */ a((O) => KO[O] || -1, "get") }],
  tokenPrec: 29354
}), BO = /* @__PURE__ */ R.define({
  name: "php",
  parser: /* @__PURE__ */ DO.configure({
    props: [
      /* @__PURE__ */ V.add({
        IfStatement: /* @__PURE__ */ e({ except: /^\s*({|else\b|elseif\b|endif\b)/ }),
        TryStatement: /* @__PURE__ */ e({ except: /^\s*({|catch\b|finally\b)/ }),
        SwitchBody: /* @__PURE__ */ a((O) => {
          let Q = O.textAfter, i = /^\s*\}/.test(Q), y = /^\s*(case|default)\b/.test(Q);
          return O.baseIndent + (i ? 0 : y ? 1 : 2) * O.unit;
        }, "SwitchBody"),
        ColonBlock: /* @__PURE__ */ a((O) => O.baseIndent + O.unit, "ColonBlock"),
        "Block EnumBody DeclarationList": /* @__PURE__ */ r({ closing: "}" }),
        ArrowFunction: /* @__PURE__ */ a((O) => O.baseIndent + O.unit, "ArrowFunction"),
        "String BlockComment": /* @__PURE__ */ a(() => null, "String BlockComment"),
        Statement: /* @__PURE__ */ e({ except: /^({|end(for|foreach|switch|while)\b)/ })
      }),
      /* @__PURE__ */ U.add({
        "Block EnumBody DeclarationList SwitchBody ArrayExpression ValueList": v,
        ColonBlock(O) {
...
function O$(O = {}) {
  let Q = [], i;
  if (O.baseLanguage !== null) if (O.baseLanguage)
    i = O.baseLanguage;
  else {
    let y = u({ matchClosingTags: !1 });
    Q.push(y.support), i = y.language;
  }
  return new b(BO.configure({
    wrap: i && m((y) => y.type.isTop ? {
      parser: i.parser,
      overlay: /* @__PURE__ */ a((S) => S.name == "Text", "overlay")
    } : null),
