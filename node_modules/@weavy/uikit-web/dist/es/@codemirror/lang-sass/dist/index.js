...
var R = (O, $) => U(O, "name", { value: $, configurable: !0 });
import { ExternalTokenizer as i, ContextTracker as V, styleTags as G, tags as e, LRParser as u, LRLanguage as b, foldNodeProp as Y, foldInside as w, indentNodeProp as _, continuedIndent as E, defineCSSCompletionSource as C, LanguageSupport as N } from "../../../editor.js";
const T = 154, P = 155, D = 156, F = 1, I = 2, Z = 3, K = 157, L = 158, p = 4, A = 5, B = 159, x = 160, v = 161, s = 162, h = 6, W = 7, M = 8, J = 9, X = 0, r = [
  9,
  10,
  11,
  12,
  ...
  8287,
  12288
], H = 58, OO = 40, m = 95, $O = 91, o = 45, eO = 46, k = 35, aO = 37, q = 123, QO = 125, d = 47, c = 42, l = 10, j = 61, tO = 43, nO = 38;
function S(O) {
...
function f(O) {
  let $;
  return O.next == d && (($ = O.peek(1)) == d || $ == c);
}
R(f, "startOfComment");
const lO = new i((O, $) => {
  if ($.dialectEnabled(X)) {
    let a;
    if (O.next < 0 && $.canShift(v))
      O.acceptToken(v);
    else if (((a = O.peek(-1)) == l || a < 0) && $.canShift(x)) {
      let Q = 0;
      for (; O.next != l && r.includes(O.next); )
        O.advance(), Q++;
      O.next == l || f(O) ? O.acceptToken(x, -Q) : Q && O.acceptToken(s);
    } else if (O.next == l)
      O.acceptToken(B, 1);
    else if (r.includes(O.next)) {
      for (O.advance(); O.next != l && r.includes(O.next); ) O.advance();
      O.acceptToken(s);
    }
  } else {
    let a = 0;
    ...
    if (O.advance(), $.dialectEnabled(X)) {
      ...
      if (a > -1) {
        ...
        for (O.advance(); O.next >= 0; )
          ...
          } else if (Q && O.next == c && O.peek(1) == d) {
            t = 2;
            break;
          } else
            O.advance();
        O.acceptToken(Q ? W : h, t);
        return;
      }
    }
    if (O.next == d) {
      for (; O.next != l && O.next >= 0; ) O.advance();
      O.acceptToken(h);
    } else {
      for (O.advance(); O.next >= 0; ) {
        let { next: a } = O;
        if (O.advance(), a == c && O.next == d) {
          O.advance();
          break;
        }
      }
  ...
  (O.next == tO || O.next == j) && $.dialectEnabled(X) && O.acceptToken(O.next == j ? M : J, 1);
}), iO = new i((O, $) => {
  if (!$.dialectEnabled(X)) return;
  let a = $.context.depth;
  if (O.next < 0 && a) {
    O.acceptToken(P);
    return;
  ...
  if (O.peek(-1) == l) {
    ...
    for (; O.next != l && r.includes(O.next); )
      O.advance(), t++;
    t != a && O.next != l && !f(O) && (t < a ? O.acceptToken(P, -t) : O.acceptToken(T));
  }
}), oO = new i((O, $) => {
  for (let a = !1, Q = 0, t = 0; ; t++) {
    let { next: n } = O;
    if (S(n) || n == o || n == m || a && g(n))
      !a && (n != o || t > 0) && (a = !0), Q === t && n == o && Q++, O.advance();
    else if (n == k && O.peek(1) == q) {
      O.acceptToken(A, 2);
      break;
    } else {
      a && O.acceptToken(n == OO ? K : Q == 2 && $.canShift(p) ? p : L);
      break;
    }
  }
}), dO = new i((O) => {
  if (O.next == QO) {
    for (O.advance(); S(O.next) || O.next == o || O.next == m || g(O.next); )
      O.advance();
    O.next == k && O.peek(1) == q ? O.acceptToken(I, 2) : O.acceptToken(F);
  }
}), SO = new i((O) => {
  if (r.includes(O.peek(-1))) {
    let { next: $ } = O;
    (S($) || $ == m || $ == k || $ == eO || $ == $O || $ == H || $ == o || $ == nO) && O.acceptToken(D);
  }
}), cO = new i((O) => {
  if (!r.includes(O.peek(-1))) {
    let { next: $ } = O;
    if ($ == aO && (O.advance(), O.acceptToken(Z)), S($)) {
      do
        O.advance();
      while (S(O.next));
      O.acceptToken(Z);
...
R(z, "IndentLevel");
const XO = new z(null, 0), sO = new V({
  start: XO,
  shift(O, $, a, Q) {
    return $ == T ? new z(O, a.pos - Q.pos) : $ == P ? O.parent : O;
  },
  hash(O) {
    return O.hash;
  }
  ...
  "[ ]": e.squareBracket,
  "{ }": e.brace
}), yO = { __proto__: null, not: 62, only: 62, using: 179, as: 189, with: 193, without: 193, hide: 207, show: 207, from: 230, to: 232, if: 245, through: 251, in: 257 }, mO = { __proto__: null, url: 80, "url-prefix": 80, domain: 80, regexp: 80, lang: 94, "nth-child": 94, "nth-last-child": 94, "nth-of-type": 94, "nth-last-of-type": 94, dir: 94, "host-context": 94, selector: 172 }, kO = { __proto__: null, "@import": 156, "@include": 176, "@mixin": 182, "@function": 182, "@use": 186, "@extend": 196, "@at-root": 200, "@forward": 204, "@media": 210, "@charset": 214, "@namespace": 218, "@keyframes": 224, "@supports": 236, "@if": 240, "@else": 242, "@for": 248, "@each": 254, "@while": 260, "@debug": 264, "@warn": 264, "@error": 264, "@return": 264 }, fO = u.deserialize({
  version: 14,
  ...
  tokenizers: [iO, SO, dO, cO, oO, lO, RO, rO, 0, 1, 2, 3],
  topRules: { StyleSheet: [0, 10], Styles: [1, 134] },
  dialects: { indented: 0 },
  specialized: [{ term: 158, get: /* @__PURE__ */ R((O) => yO[O] || -1, "get") }, { term: 157, get: /* @__PURE__ */ R((O) => mO[O] || -1, "get") }, { term: 77, get: /* @__PURE__ */ R((O) => kO[O] || -1, "get") }],
  tokenPrec: 3027
}), y = /* @__PURE__ */ b.define({
  name: "sass",
...
}), ZO = /* @__PURE__ */ y.configure({
  dialect: "indented",
  props: [
    /* @__PURE__ */ _.add({
      "Block RuleSet": /* @__PURE__ */ R((O) => O.baseIndent + O.unit, "Block RuleSet")
    }),
    /* @__PURE__ */ Y.add({
      Block: /* @__PURE__ */ R((O) => ({ from: O.from, to: O.to }), "Block")
    })
  ]
}), pO = /* @__PURE__ */ C((O) => O.name == "VariableName" || O.name == "SassVariableName");
function hO(O) {
  return new N(O != null && O.indented ? ZO : y, y.data.of({ autocomplete: pO }));
}
R(hO, "sass");
export {
  hO as sass,
  pO as sassCompletionSource,
  y as sassLanguage
};
